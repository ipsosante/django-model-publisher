...
class PublisherForm(forms.ModelForm):
    def clean(self):
        ...
        for unique_fields in unique_fields_set:
            ...
            existing_instances = (
                type(instance).objects.filter(**unique_filter).exclude(pk=instance.pk)
            )
...
class PublisherAdmin(ModelAdmin):
    ...
    class Media:
        ...
        css = {"all": ("publisher/publisher.css",)}
    ...
    def get_model_object(self, request, object_id):
        ...
        if obj is None:
            raise Http404(
                ...
                % (force_text(self.model._meta.verbose_name), escape(object_id))
            )

        if not self.has_change_permission(request) and not self.has_add_permission(
            request
        ):
            raise PermissionDenied
